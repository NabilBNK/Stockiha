...
| **WS-H** | Backup & Recovery | **Repaired and completed for installed builds — WS-H-3..WS-H-7 accepted on <to be filled by the Owner>** | WS-H-1/WS-H-2's `run.bat`-only, command-line-restore design (described in the row this replaced) is superseded for installed (embedded) builds by WS-H-3 through WS-H-7. What now exists for an embedded install: manual backups with checksum/integrity validation; a backup list with copy-to-another-drive; an isolated restore test against a throwaway database; in-app restore with an automatic safety copy and rollback on failure; restore on a new PC (first-run setup screen, before any account exists); a mandatory backup before every update install; and a daily local automatic backup with retention (14 daily / 5 pre-update / 5 pre-restore, manual never deleted). Arabic and French translations complete (WS-H-7). The developer/`run.bat` command-line procedure ([`docs/recovery/RESTORE_PROCEDURE.md`](./docs/recovery/RESTORE_PROCEDURE.md)) still exists and is now documented as the developer-only path — it is not used by installed builds at all. |
| **WS-K** | Windows Desktop Acceptance | In Progress — embedded PostgreSQL (WS-K-4); first-run setup **confirmed on a fresh PC** at WS-K-4.6; orphan-on-close fixed in WS-K-4.7, awaiting retest | WS-K-1/1.2 unchanged and still authoritative: production `database.json` config resolution (env var → per-install config file → dev `runtime.key`), the ten-state startup-failure screen, fail-open schema-version check. **WS-K-2 and WS-K-3's Windows-service provisioning design is retired**, not merely superseded — three consecutive real-machine installs on that line failed (`$PSScriptRoot`-empty, wrong resource-path math, `SetShellVarContext all`/`$APPDATA` bugs, each fixed in turn — and the install *still* failed after all three fixes), with the leading suspect being `New-StockihaServiceAccount`/`Grant-LogOnAsServiceRight`'s `LsaAddAccountRights` P/Invoke, which real-machine testing never once actually exercised successfully. **WS-K-4 replaces the whole service-account architecture**: PostgreSQL now runs as a plain child process of Stockiha itself (`src-tauri/src/infrastructure/pg_process.rs`), started/stopped by the app, no Windows service, no dedicated service account, no elevation. First-run provisioning (`src-tauri/src/infrastructure/embedded_setup.rs`) moved from an unattended PowerShell script run once during install into the app itself, visible on screen (`EmbeddedSetupScreen.tsx`) with a live 9-step checklist and a working Retry that resumes without reinstalling. `Provision-StockihaPostgres.ps1`, `Uninstall-StockihaPostgres.ps1`, `Test-ProvisionInvocationSmoke.ps1`, and `src-tauri/nsis/hooks.nsh` are all **deleted** — the installer's only remaining job is copying files (`installMode` is now `"currentUser"`, no elevation). All 7 required scenarios (full setup end-to-end, stale-pid dead/live, post-exit process cleanup, migration-failure leaves no `database.json`, no password in `setup.log`, and — the highest-probability remaining risk — a non-ASCII **Arabic** data path) were run for real against the bundled PostgreSQL 18.6 binaries; the Arabic-path case failed on first attempt (`initdb` truncates non-ASCII path characters via its own ANSI-codepage argument handling) and now falls back to a fixed ASCII path under `%ProgramData%\Stockiha\pgdata` when `app_data_dir` is not ASCII-safe (`pg_process::resolve_pgdata_dir`). **WS-K-4.2 fixes the packaging bug that made WS-K-4.1's install fail on the Owner's real machine**, found by the Owner inspecting the installed folder directly: `tauri.conf.json`'s resource mapping used a glob (`resources/postgres/win64/**/*` → `postgres/win64/`), and in map form a glob flattens every matched file into the target directory **by basename** (`tauri_utils::resources` computes the target as `dest.join(path.file_name())`). `bin/`, `lib/` and `share/` therefore did not exist in the installed app at all, and 1,565 source files collapsed to 1,100 — 465 silently overwrote each other, so the shipped PostgreSQL was corrupted, not merely misplaced. Mapping the directory itself (`"resources/postgres/win64": "postgres/win64"`) takes the structure-preserving walk branch instead. Three new non-ignored tests now guard it: the materialized (staged) resource layout is asserted against what the shipped code reads, the config is asserted to contain no glob patterns, and the missing-binary diagnostic is asserted to name the exact path searched plus a listing of the nearest folder that does exist — all three verified to **fail** against the old mapping. The real-PostgreSQL tests were also repointed from the pristine source tree to the staged tree, since reading the source tree is precisely why they stayed green throughout this bug. **WS-K-4.3 fixes the second real-machine failure**, found by the Owner running WS-K-4.2's installer on a fresh PC: setup failed at `InitializeDatabase` with `initdb exited with exit code: 0xc0000135` and no error text, and Windows' own dialog named the cause — `VCRUNTIME140.dll est introuvable`. `0xC0000135` is `STATUS_DLL_NOT_FOUND`: the bundled PostgreSQL binaries are MSVC-built and link against the Visual C++ 2015-2022 Redistributable (154 of them import `vcruntime140.dll`, 12 `vcruntime140_1.dll`, 9 `msvcp140.dll`), which is **not** part of a clean Windows install. EDB's own installer installs that redistributable as one of its steps; bundling the raw zip binaries skipped it. Every development machine worked because Visual Studio's Build Tools leave those DLLs in `System32` — so no local test could ever have caught it by running the binaries. Fixed by deploying the three DLLs **app-local** in `postgres/win64/bin/` (the executable's own directory is first in Windows' DLL search order), rather than running `vc_redist.x64.exe`, which would require elevation and break WS-K-4's no-admin guarantee; all three are Authenticode-signed and verify `Valid`, a stronger provenance chain than the PostgreSQL binaries themselves. Guarded by a new test that parses the **PE import tables** of every bundled executable and fails if any Visual C++ runtime import is absent from the bundle — environment-independent by construction, and verified to fail against the previous build. Also fixed in the same pass, both visible in the Owner's screenshots: `initdb`/`postgres`/`pg_ctl` now spawn with `CREATE_NO_WINDOW` (a black console window was appearing mid-setup, which this document explicitly promises will never happen), and load-failure exit codes are now translated into plain language instead of surfacing as bare hex. **WS-K-4.4 fixes the third real-machine failure**, again found on the fresh PC: with the runtime DLLs in place `initdb` finally executed, and then failed with `program "postgres" is needed by initdb but was not found in the same directory as "//?/C:/Users/pc marhaba/.../bin/initdb.exe"` — while `postgres.exe` sat in that exact directory. Cause: Tauri's `resource_dir()` canonicalizes `current_exe()` (a deliberate symlink-resolution security measure on its part), and on Windows `Path::canonicalize` always returns the **`\\?\` verbatim form**. Windows accepts it everywhere and `CreateProcess` launched `initdb` from it fine, but PostgreSQL's own path handling converts `argv[0]` to forward slashes (`//?/C:/...`), derives its own directory from that, and shells out via `cmd.exe` to run `"<dir>/postgres.exe" -V` — which `cmd.exe` cannot resolve. Fixed with `dunce::simplified` (already in the lock file via Tauri, so no new compilation) in a single new `pg_process::bundled_bin_dir` helper that both call sites now use, which also collapses the duplicated `postgres/win64/bin` expression that let the WS-K-4.2 bug be wrong in two places at once. **The deeper fix is to the tests**: this was the third consecutive bug where the tests resolved the bundle path differently from production — the helpers now canonicalize exactly as Tauri does and route through production's own path builder, so the real-PostgreSQL tests receive the same `\\?\` path the shipped app gets. Verified by reverting the fix and watching the end-to-end test reproduce the Owner's error byte-for-byte locally. **WS-K-4.5 addresses the fourth real-machine failure**, this time from an install on the `D:` drive: `initdb` reported `invalid binary "D:\?\Stockiha\...\initdb.exe"`, `could not re-execute with restricted token: error code 2`, and finally `program "postgres" is needed by initdb but was not found in the same directory as "initdb"`. `?` is an illegal character in a Windows filename, so that path is a mangled `\\?\` verbatim path — meaning a verbatim path was still reaching `initdb` despite WS-K-4.4's `dunce::simplified` call. Rather than keep stripping a prefix after the fact, WS-K-4.5 stops creating it: the bundle is now resolved from `std::env::current_exe()` (which does **not** canonicalize) instead of Tauri's `resource_dir()` (which does, deliberately, for symlink resolution), with `dunce` retained as a second layer and Tauri's resolver kept only as a fallback. This also covers the case `dunce` intentionally refuses — `\\?\UNC\…`, what a canonicalized path on a mapped network drive looks like. `preflight_bundle_layout` now refuses a verbatim `bin` directory outright, with an explanation, rather than letting PostgreSQL fail three layers later in its own vocabulary. Two diagnostic gaps that made this round trip necessary are also closed: `setup.log` now records the exact resource/bin/data directories before anything runs, the `initdb` failure message names the binary Stockiha actually launched (its own messages describe the path *it* derived, which is not the same thing), and **the build version is now shown on the setup screen**, not only on the dashboard — which is unreachable precisely when setup fails. **WS-K-4.6 (WS-K-MERGE) merges `main` into this branch** — main's four commits since the WS-K-3.3 fork (WS-E-1 purchases redesign, WS-E-2 supplier payments, WS-E-3 supplier returns, and a settings-page UI pass) now ride on top of the embedded-PostgreSQL work, bringing the migration set from 147 to 149. One conflict: the dashboard version line, resolved to ws-k-4's `{APP_VERSION_MARKER}` form (main's literal would have been a dead string — main has no `version.ts`, and the setup screen's version display depends on that single source). The two WS-E migrations had never run against the embedded instance; the end-to-end setup test now asserts, through the app's own production schema check, that a fresh embedded instance ends `UpToDate` at the binary's latest migration, and prints the count — `embedded=149 applied_rows=149 verdict=UpToDate`. Merging in this direction on purpose: this is the branch validated on the Owner's real PC across five rounds, and it stays the branch under test; it merges back to `main` only after the Owner validates the merged build. **WS-K-4.6 is the first build to complete first-run setup on the Owner's fresh PC** — version confirmed on the setup screen, all nine steps done, close/reopen took the fast path. It also surfaced the next defect: "PostgreSQL Server" still in Task Manager after closing Stockiha. Root cause (`WS-K-4.7`): `tauri::process::restart` after setup calls `std::process::exit(0)`, which never reaches the event loop's `RunEvent::Exit` — the only shutdown hook — and Windows does not kill children with their parent, so the setup-time `postgres.exe` outlived its process; the relaunched app then found a live `postmaster.pid` and, correctly by the stale-pid rule, refused to manage a server it had not spawned, so its own shutdown had nothing to stop and every later launch inherited the orphan (which is exactly why close/reopen "worked"). Fixed twice over: the setup command now stops its server before relaunching, and the exit hook now also stops any live server on Stockiha's data directory whether or not this process spawned it (safe: the directory is exclusively Stockiha's, single-instance is enforced, and the developer cluster lives elsewhere) — which self-heals the orphan already running on the Owner's PC on the next normal close. Reproduced in a new real-PostgreSQL test that spawns a server, `mem::forget`s the handle exactly as `exit(0)` does, and proves the exit-hook logic still stops it through the data directory alone. **The `WS-K-4.7` installer has not been run on a fresh machine** — **WS-K-4.8 merges `task/ws-f-3-sale-discount` into this branch** (WS-F-2 receipt printing / printing settings / attribute receipt visibility, and WS-F-3 sale discount — committed as `c404d9e` first, since that work had been sitting uncommitted). Migrations 149 → 152, the three WS-F files on top, ascending, no duplicates; the end-to-end setup test now reports `embedded=152 applied_rows=152 verdict=UpToDate`. One conflict, the dashboard version line, resolved as before. Vitest 468 → 504. **WS-K-4.9 makes upgrades work.** Installing 4.8 over 4.7 on the fresh PC hit WS-K-1's "Database needs an update — contact your supplier" gate: the schema was 149 and the binary expected 152. Correct for the externally managed database WS-K-1 was written for; wrong for an embedded one that Stockiha owns. Root cause underneath: setup persisted only `stockiha_runtime` (least privilege, by design) and discarded the migrator password after using it once, so **no install could ever apply a later build's migrations** — also the WS-K-4.1 "repair path cannot migrate" gap. Fixed by persisting the migrator credential in a separate `migrator.json` (`database.json`'s WS-K-1 format untouched; the superuser password is still never persisted) and applying pending migrations at startup, before the WS-K-1 diagnostic runs; SQLx still refuses a newer-than-binary database, so the downgrade screen is unchanged. The end-to-end test now also proves the persisted credential connects and drives the migrator (`UPGRADE PATH: ... Some(UpToDate)`). Installs set up by 4.6–4.8 have no migrator credential and need a one-time app-data reset (documented; those builds only ever existed on the Owner's test PC). Installers are now named `Stockiha_<marker>-setup.exe` via `npm run tauri:build`. **Not merged to `main`** — that waits for the Owner's signal after testing this build. see `WS-K-4-MANUAL-VERIFICATION.md`; the Task Manager check after a normal close is the specific thing to verify next. |
