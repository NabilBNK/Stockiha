...
| **WS-K** | Windows Desktop Acceptance | In Progress — installer built, unverified on a clean VM | WS-K-1/1.2: production `database.json` config resolution (env var → per-install config file → dev `runtime.key`), ten-state startup-failure screen, fail-open schema-version check — landed and hotfixed after a real permission-denied incident on the Owner's own cluster. WS-K-2: unattended provisioning design (detection, silent install, role/grant creation, `database.json` writing) delivered as PowerShell source, **never executed** — no live install, service registration, or role creation has been run anywhere. WS-K-3: migrations now run via the app's own `--provision-migrate` flag (embedded SQLx migrator) instead of a separately bundled `sqlx.exe`; a real, working NSIS installer (`Stockiha_0.1.0_x64-setup.exe`) has been built via `npm run tauri build`, with PostgreSQL 18.6 Windows binaries bundled (provenance: EDB's installer wrapper for this release is Authenticode-signed by EnterpriseDB Corporation — verified; the individual bundled binaries themselves are unsigned and were not extracted from inside that signed wrapper, so this is same-version/same-host correspondence, not a checksum or signature match — see `resources/postgres/win64/README.md`'s verification log for the full, honest breakdown). **The installer has not been run on any machine, clean VM or otherwise** — see `WS-K-3-MANUAL-VERIFICATION.md` for the Owner's own test script; that is the next required action before WS-K can move to Confirmed. |
