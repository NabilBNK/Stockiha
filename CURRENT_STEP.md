...
| **WS-H** | Backup & Recovery | **Not acceptance-passed — parked after WS-H-2** | WS-H-2 fixed: the misleading "permission denied" on validate/restore (validate/verify resolved the backup root from the env var only while create used the stored `r6_003` setting; the mismatch was reported as `PermissionDenied` and now has its own `BACKUP_BUNDLE_OUTSIDE_ROOT` code), native folder pickers for the backup destination and for an existing bundle, the restore-card layout (`align-self: end` — a grid affordance — was shoving both checkboxes to the right margin inside a flex column; heading and body-copy scale also mismatched), the stale `r6_002` schema-version assertion, and a startup diagnostic that names a missing `STOCKIHA_BACKUP_ROOT` / `STOCKIHA_RESTORE_ADMIN_DATABASE_URL` instead of letting it surface later as a message that reads like a broken feature. Automated suites green (268 Rust unit tests, frontend suite except the two pre-existing WS-E failures, all 4 recovery SQL suites). **Windows manual acceptance has never passed and is still required**, and must be run via `run.bat` — a bare `npm run tauri dev` does not export the recovery environment. |
