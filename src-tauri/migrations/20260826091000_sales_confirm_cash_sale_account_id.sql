-- WS-B-1 Gate 2: convert sales.confirm_cash_sale to dual-write account_id
-- alongside account_code. Also fixes a pre-existing, never-exercised defect:
-- this function hardcoded the account_role_code enum value 'CASH' directly
-- into account_code instead of the actual legacy code 'CASH_DESK' every
-- other function and finance.accounts use (finance.require_account_role('CASH')
-- has always correctly returned 'CASH_DESK' -- only this literal bypassed
-- that lookup). Fixing the literal itself, rather than resolving the wrong
-- string, keeps account_code and account_id referencing the same account --
-- the entire point of the dual-write safety mechanism.
--
-- No other line of this function changes: same WAC logic, same permission
-- check, same idempotency handling, same COGS snapshot logic.
...
BEGIN
    ...
    -- 9. Balanced journal -- Cash sale revenue (Dr CASH_DESK / Cr SALES_REVENUE)
    ...
    INSERT INTO finance.journal_lines (document_id, line_number, account_code, account_id, debit, credit) VALUES
        (v_journal_document_id, 1, 'CASH_DESK', finance.resolve_account_id('CASH_DESK'), v_subtotal, 0),
