-- S4-001: Atomic customer credit sale posting with concurrency-safe exposure enforcement.
...
BEGIN
    ...
    FROM iam.resolve_session_with_permission(p_session_token, 'OVERRIDE_CREDIT_LIMIT');

    IF p_token_id IS NULL THEN
        RAISE EXCEPTION 'override token id is required' USING ERRCODE = '22023';
    END IF;
    IF p_payload_hash IS NULL OR octet_length(p_payload_hash) = 0 THEN
        RAISE EXCEPTION 'override payload hash is required' USING ERRCODE = '22023';
    END IF;
    IF v_reason IS NULL THEN
        RAISE EXCEPTION 'override authorization reason is required' USING ERRCODE = '22023';
    END IF;
    IF p_ttl_minutes IS NULL OR p_ttl_minutes < 1 OR p_ttl_minutes > 60 THEN
        RAISE EXCEPTION 'override ttl must be between 1 and 60 minutes' USING ERRCODE = '22023';
    END IF;

    PERFORM 1
    FROM receivables.customers
    WHERE id = p_customer_id AND is_active AND credit_enabled;
    IF NOT FOUND THEN
        RAISE EXCEPTION 'customer is inactive, missing, or not credit-enabled' USING ERRCODE = '55000';
    END IF;

    INSERT INTO receivables.credit_override_tokens (
        id,
        customer_id,
...
BEGIN
    ...
    INSERT INTO finance.journal_lines (document_id, line_number, account_code, debit, credit) VALUES
        (v_journal_document_id, 1, 'ACCOUNTS_RECEIVABLE', v_subtotal, 0),
