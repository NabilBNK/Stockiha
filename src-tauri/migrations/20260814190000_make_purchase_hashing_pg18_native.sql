-- Migration: 20260814190000_make_purchase_hashing_pg18_native.sql
-- Stockiha Purchase Transaction Contract Repair:
-- Make all nested request idempotency hashing native to PostgreSQL 18.
-- Replaces pgcrypto/custom core.digest references inside procurement.post_purchase_transaction
-- with native pg_catalog sha256(convert_to(..., 'UTF8')).
...
-- Verification DO block to ensure sha256(convert_to(..., 'UTF8')) executes cleanly
DO $$
DECLARE
    v_test_hash bytea;
BEGIN
    v_test_hash := sha256(convert_to('stockiha-native-pg18-purchase-hashing-test', 'UTF8'));
    IF length(v_test_hash) <> 32 THEN
        RAISE EXCEPTION 'Native SHA-256 test failed: expected 32 bytes, got %', length(v_test_hash);
    END IF;
END;
$$;
