...
CREATE TABLE receivables.customer_payments (
    ...
    CONSTRAINT customer_payment_method_valid CHECK (payment_method IN ('CASH', 'BANK_TRANSFER', 'CHECK')),
    CONSTRAINT customer_payment_amount_positive CHECK (amount > 0),
    CONSTRAINT customer_payment_cash_session_consistent CHECK (
        (payment_method = 'CASH' AND cash_session_id IS NOT NULL)
        OR (payment_method <> 'CASH' AND cash_session_id IS NULL)
    ),
    CONSTRAINT customer_payment_workstation_not_blank CHECK (btrim(workstation_id) <> '')
);

CREATE TABLE receivables.payment_allocations (
    id                       bigint GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    payment_document_id      bigint NOT NULL REFERENCES receivables.customer_payments (document_id),
    ...
    CONSTRAINT payment_allocation_invoice_unique UNIQUE (payment_document_id, invoice_ledger_entry_id)
);

CREATE INDEX payment_allocations_invoice_idx
    ON receivables.payment_allocations (invoice_ledger_entry_id);

CREATE FUNCTION receivables.forbid_payment_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    RAISE EXCEPTION 'posted customer payments and allocations are immutable'
        USING ERRCODE = '0A000';
END;
$$;

CREATE TRIGGER customer_payments_forbid_update
    BEFORE UPDATE ON receivables.customer_payments
    FOR EACH ROW EXECUTE FUNCTION receivables.forbid_payment_mutation();
CREATE TRIGGER customer_payments_forbid_delete
    BEFORE DELETE ON receivables.customer_payments
    FOR EACH ROW EXECUTE FUNCTION receivables.forbid_payment_mutation();
CREATE TRIGGER payment_allocations_forbid_update
    BEFORE UPDATE ON receivables.payment_allocations
    FOR EACH ROW EXECUTE FUNCTION receivables.forbid_payment_mutation();
CREATE TRIGGER payment_allocations_forbid_delete
    BEFORE DELETE ON receivables.payment_allocations
    FOR EACH ROW EXECUTE FUNCTION receivables.forbid_payment_mutation();

CREATE FUNCTION receivables.post_customer_payment(
...
BEGIN
    ...
    END IF;
    IF v_method NOT IN ('CASH', 'BANK_TRANSFER', 'CHECK') THEN
    ...
    -- Fingerprint actual posting fields. JSONB canonical text prevents caller hash substitution.
    ...
    -- Same customer row lock used by credit sales. Payment and new sale cannot race
    -- exposure decisions against stale values.
    ...
    -- Lock every targeted invoice ledger row in deterministic order, then prove
    -- same customer + credit invoice + remaining amount.
    PERFORM 1
    FROM receivables.customer_ledger_entries l
    WHERE l.id IN (
        SELECT DISTINCT (elem ->> 'invoice_ledger_entry_id')::bigint
    ...
    LOOP
        v_invoice_entry_id := nullif(v_alloc ->> 'invoice_ledger_entry_id', '')::bigint;
        v_alloc_amount := nullif(v_alloc ->> 'amount', '')::numeric;
        IF v_invoice_entry_id IS NULL OR v_alloc_amount IS NULL OR v_alloc_amount <= 0 THEN
...
REVOKE ALL ON receivables.customer_payments FROM PUBLIC;
REVOKE ALL ON receivables.payment_allocations FROM PUBLIC;
REVOKE ALL ON receivables.customer_payments FROM stockiha_runtime;
REVOKE ALL ON receivables.payment_allocations FROM stockiha_runtime;
REVOKE ALL ON FUNCTION receivables.forbid_payment_mutation() FROM PUBLIC;
