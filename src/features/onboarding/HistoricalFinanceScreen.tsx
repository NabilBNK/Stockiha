...
const COPY: Record<Locale, Record<string, string>> = {
  en: {
    ...
    excelTitle: 'Primary path — Excel import',
    excelHelp: 'Use the official Stockiha workbook with Historical_Transactions and Balances sheets. Formulas are rejected; only entered values are accepted.',
  ...
  fr: {
    ...
    excelTitle: 'Chemin principal — import Excel',
    excelHelp: 'Utilisez le classeur Stockiha officiel avec les feuilles Historical_Transactions et Balances. Les formules sont refusées.',
  ...
  ar: {
    ...
    excelTitle: 'المسار الرئيسي — استيراد Excel',
    excelHelp: 'استعمل ملف Stockiha الرسمي الذي يحتوي على Historical_Transactions وBalances. الصيغ الحسابية غير مقبولة.',
...
function nextRequestId(source: 'excel' | 'manual'): string {
  requestSequence += 1;
...
export function HistoricalFinanceScreen({ sessionToken }: Props) {
  ...
  async function stageAndValidateExcel() {
    if (!canImport || !selectedFile || !workbook) return;
    setBusy('import');
    setError(null);
    setFeedback(null);
    setValidation(null);
  ...
  return (
    <section className="sk-page" aria-labelledby="historical-finance-title">
      <div className="sk-card">
        <h1 id="historical-finance-title">{text.title}</h1>
        <p>{text.subtitle}</p>
        ...
        <div className="sk-field">
          <label className="sk-field__label" htmlFor="historical-import-enabled">
            <input
              id="historical-import-enabled"
              type="checkbox"
              checked={enabled}
              disabled={busy !== null}
