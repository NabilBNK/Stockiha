...
const COPY: Record<Locale, Record<string, string>> = {
  en: {
    title: 'Customers', newCustomer: 'New customer', editCustomer: 'Edit customer', code: 'Code', name: 'Name',
    contact: 'Contact', phone: 'Phone', email: 'Email', taxId: 'Tax ID', address: 'Address', active: 'Active',
    inactive: 'Inactive', credit: 'Credit', cashOnly: 'Cash only', creditEnabled: 'Credit enabled',
    creditLimit: 'Credit limit', terms: 'Payment terms (days)', maxOverdue: 'Maximum overdue days', exposure: 'Exposure',
    available: 'Available credit', oldestDue: 'Oldest open due date', overdueBlocked: 'Overdue block', ledger: 'Customer ledger',
    entry: 'Entry', amount: 'Amount', due: 'Due date', document: 'Document', created: 'Created', view: 'View', edit: 'Edit',
    activate: 'Activate', deactivate: 'Deactivate', save: 'Save', cancel: 'Cancel', none: 'No customers yet.',
    noLedger: 'No ledger entries yet.', yes: 'Yes', no: 'No', readOnly: 'Read-only customer access',
  },
  fr: {
    title: 'Clients', newCustomer: 'Nouveau client', editCustomer: 'Modifier le client', code: 'Code', name: 'Nom',
    contact: 'Contact', phone: 'Téléphone', email: 'E-mail', taxId: 'NIF / NIS', address: 'Adresse', active: 'Actif',
    inactive: 'Inactif', credit: 'Crédit', cashOnly: 'Comptant uniquement', creditEnabled: 'Crédit autorisé',
    creditLimit: 'Plafond de crédit', terms: 'Délai de paiement (jours)', maxOverdue: 'Retard maximum (jours)', exposure: 'Encours',
    available: 'Crédit disponible', oldestDue: 'Plus ancienne échéance ouverte', overdueBlocked: 'Blocage pour retard', ledger: 'Grand livre client',
    entry: 'Écriture', amount: 'Montant', due: 'Échéance', document: 'Document', created: 'Créé le', view: 'Voir', edit: 'Modifier',
    activate: 'Activer', deactivate: 'Désactiver', save: 'Enregistrer', cancel: 'Annuler', none: 'Aucun client.',
    noLedger: 'Aucune écriture client.', yes: 'Oui', no: 'Non', readOnly: 'Accès client en lecture seule',
  },
  ar: {
    title: 'العملاء', newCustomer: 'عميل جديد', editCustomer: 'تعديل العميل', code: 'الرمز', name: 'الاسم',
    contact: 'جهة الاتصال', phone: 'الهاتف', email: 'البريد الإلكتروني', taxId: 'الرقم الجبائي', address: 'العنوان', active: 'نشط',
    inactive: 'غير نشط', credit: 'الائتمان', cashOnly: 'نقداً فقط', creditEnabled: 'السماح بالائتمان',
    creditLimit: 'حد الائتمان', terms: 'أجل الدفع بالأيام', maxOverdue: 'أقصى تأخر بالأيام', exposure: 'الدين الحالي',
    available: 'الائتمان المتاح', oldestDue: 'أقدم استحقاق مفتوح', overdueBlocked: 'حظر بسبب التأخر', ledger: 'حساب العميل',
    entry: 'الحركة', amount: 'المبلغ', due: 'الاستحقاق', document: 'المستند', created: 'تاريخ الإنشاء', view: 'عرض', edit: 'تعديل',
    activate: 'تفعيل', deactivate: 'تعطيل', save: 'حفظ', cancel: 'إلغاء', none: 'لا يوجد عملاء.',
    noLedger: 'لا توجد حركات في حساب العميل.', yes: 'نعم', no: 'لا', readOnly: 'صلاحية عرض العملاء فقط',
  },
};
...
export function CustomersScreen({ sessionToken }: Props) {
  ...
  async function saveCustomer(event: FormEvent) {
    ...
    try {
      if (editing) {
        await updateCustomer(sessionToken, { ...basePayload, customer_id: editing.id, is_active: editing.is_active });
      } else {
    ...
    <section className="sk-screen" data-testid="customers-screen">
      <header className="sk-screen__header">
        <div>
          ...
          {!loading && !canManage ? <small data-testid="customers-read-only">{text.readOnly}</small> : null}
        </div>
        {canManage ? (
          <button type="button" className="sk-button sk-button--primary" onClick={() => resetForm()} data-testid="add-customer-btn">
            {text.newCustomer}
          </button>
        ) : null}
      </header>
      ...
      {showForm && canManage ? (
        <form className="sk-card sk-form" onSubmit={saveCustomer} data-testid="customer-form">
          ...
          <div className="sk-form-actions">
            <button type="button" className="sk-button sk-button--secondary" onClick={() => setShowForm(false)}>{text.cancel}</button>
            <button type="submit" className="sk-button sk-button--primary" disabled={!formValid || busy} data-testid="save-customer-btn">{text.save}</button>
          </div>
        </form>
      ) : null}
      ...
      <div className="sk-card">
        {loading ? <div>{text.title}…</div> : (
          <table className="sk-table" data-testid="customers-table">
            ...
            <tbody>
              {customers.length === 0 ? <tr><td colSpan={8}>{text.none}</td></tr> : customers.map((customer) => (
                <tr key={customer.id} data-testid={`customer-row-${customer.id}`}>
                  <td><strong>{customer.code}</strong></td><td>{customer.name}</td>
                  <td>{customer.credit_enabled ? text.credit : text.cashOnly}</td><td>{customer.credit_limit}</td>
                  <td>{customer.exposure_amount}</td><td>{customer.available_credit}</td><td>{customer.is_active ? text.active : text.inactive}</td>
                  <td>
                    <button type="button" className="sk-button sk-button--small" onClick={() => void openFinancialDetail(customer)}>{text.view}</button>
                    {canManage ? <>{' '}<button type="button" className="sk-button sk-button--small" onClick={() => resetForm(customer)}>{text.edit}</button>{' '}<button type="button" className="sk-button sk-button--small" disabled={busy} onClick={() => void toggleActive(customer)}>{customer.is_active ? text.deactivate : text.activate}</button></> : null}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {selected ? (
        <div className="sk-card" data-testid="customer-financial-detail">
          <h2>{selected.name}</h2>
          {summary ? <div className="sk-form-grid">
          ...
          <table className="sk-table" data-testid="customer-ledger-table">
            ...
            <tbody>{ledger.length === 0 ? <tr><td colSpan={5}>{text.noLedger}</td></tr> : ledger.map((entry) => (
              <tr key={entry.id}><td>{entry.entry_type}</td><td>{entry.amount_delta}</td><td>{entry.due_date ?? '—'}</td><td>{entry.document_id ?? '—'}</td><td>{entry.created_at}</td></tr>
            ))}</tbody>
          </table>
        </div>
      ) : null}
