...
import {
  authorizeCustomerPaymentRefund,
  listOpenCustomerInvoices,
  listRefundableCustomerPayments,
  postCustomerPayment,
  postCustomerRefund,
} from '../../shared/ipc/receivablesGateway';
import type {
  CustomerPaymentResult,
  CustomerRefundResult,
  OpenCustomerInvoice,
  RefundableCustomerPayment,
} from '../../shared/ipc/receivablesDto';
...
const COPY: Record<Locale, Record<string, string>> = {
  en: {
    ...
    email: 'Email',
    taxId: 'Tax ID',
    taxIdHelp: 'Customer fiscal or tax registration number.',
    address: 'Address',
    active: 'Active',
    inactive: 'Inactive',
    credit: 'Credit',
    ...
    paymentTotal: 'Allocated payment total',
    noOpenInvoices: 'No open invoices.',
    paymentPosted: 'Customer payment posted',
    cashSessionRequired: 'Open a cash session before recording a cash customer payment.',
    refund: 'Refund payment',
    refundablePayments: 'Refundable payments',
    noRefundablePayments: 'No posted customer payment is available for refund.',
    sourcePayment: 'Source payment',
    refundMethod: 'Refund method',
    refundReason: 'Manager authorization reason',
    managerUsername: 'Manager username',
    managerPassword: 'Manager password',
    postRefund: 'Authorize and post refund',
    refundPosted: 'Customer payment refund posted',
    refundCashRequired: 'Open a cash session before issuing a cash refund.',
    refundFieldsRequired: 'Choose a payment and enter manager credentials and an authorization reason.',
    refundBoundary: 'This reverses the customer payment only. Product return and stock restoration remain a separate workflow.',
  },
  fr: {
    title: 'Clients',
    ...
    email: 'E-mail',
    taxId: 'NIF / NIS',
    taxIdHelp: "Identifiant fiscal ou d'immatriculation fiscale du client.",
    address: 'Adresse',
    active: 'Actif',
    inactive: 'Inactif',
    credit: 'Crédit',
    ...
    note: 'Note',
    postPayment: 'Valider le paiement',
    paymentTotal: 'Total affecté',
    noOpenInvoices: 'Aucune facture ouverte.',
    paymentPosted: 'Paiement client enregistré',
    cashSessionRequired: 'Ouvrez une session de caisse avant un paiement client en espèces.',
    refund: 'Rembourser un paiement',
    refundablePayments: 'Paiements remboursables',
    noRefundablePayments: 'Aucun paiement client validé ne peut être remboursé.',
    sourcePayment: 'Paiement source',
    refundMethod: 'Mode de remboursement',
    refundReason: "Motif d'autorisation du responsable",
    managerUsername: 'Utilisateur responsable',
    managerPassword: 'Mot de passe responsable',
    postRefund: 'Autoriser et valider le remboursement',
    refundPosted: 'Remboursement du paiement client enregistré',
    refundCashRequired: 'Ouvrez une session de caisse avant un remboursement en espèces.',
    refundFieldsRequired: 'Choisissez un paiement et saisissez les identifiants du responsable ainsi que le motif.',
    refundBoundary: 'Cette opération annule uniquement le paiement client. Le retour produit et la remise en stock restent un flux séparé.',
  },
  ar: {
    title: 'العملاء',
    ...
    taxId: 'الرقم الجبائي',
    taxIdHelp: 'رقم التعريف أو التسجيل الجبائي للعميل.',
    address: 'العنوان',
    active: 'نشط',
    inactive: 'غير نشط',
    credit: 'الائتمان',
    ...
    bank: 'تحويل بنكي',
    note: 'ملاحظة',
    postPayment: 'تسجيل الدفع',
    paymentTotal: 'إجمالي التخصيص',
    noOpenInvoices: 'لا توجد فواتير مفتوحة.',
    paymentPosted: 'تم تسجيل دفعة العميل',
    cashSessionRequired: 'افتح جلسة الصندوق قبل تسجيل دفعة نقدية من العميل.',
    refund: 'استرجاع دفعة',
    refundablePayments: 'الدفعات القابلة للاسترجاع',
    noRefundablePayments: 'لا توجد دفعة عميل مسجلة قابلة للاسترجاع.',
    sourcePayment: 'الدفعة الأصلية',
    refundMethod: 'طريقة الاسترجاع',
    refundReason: 'سبب موافقة المسؤول',
    managerUsername: 'اسم مستخدم المسؤول',
    managerPassword: 'كلمة مرور المسؤول',
    postRefund: 'الموافقة وتسجيل الاسترجاع',
    refundPosted: 'تم تسجيل استرجاع دفعة العميل',
    refundCashRequired: 'افتح جلسة الصندوق قبل استرجاع المبلغ نقداً.',
    refundFieldsRequired: 'اختر الدفعة وأدخل بيانات المسؤول وسبب الموافقة.',
    refundBoundary: 'هذه العملية تعكس دفعة العميل فقط. إرجاع المنتج وإعادة المخزون يبقيان مساراً منفصلاً.',
  },
};
...
export function CustomersScreen({ sessionToken }: Props) {
  ...
  const { activeCashSession, workstationId } = useSession();
  ...
  const [refundablePayments, setRefundablePayments] = useState<RefundableCustomerPayment[]>([]);
  ...
  const [lastRefund, setLastRefund] = useState<CustomerRefundResult | null>(null);

  const canManage = capabilities?.can_manage_customers === true;
  const canPostPayment = capabilities?.can_post_customer_payment === true;
  const canPostRefund = capabilities?.can_post_customer_refund === true;

  const selectedRefundPayment = useMemo(
    () => refundablePayments.find((payment) => payment.payment_document_id === refundPaymentId) ?? null,
    [refundablePayments, refundPaymentId],
  );
  ...
  async function loadCustomers() {
    ...
    setError(null);
    try {
      const [customerRows, access] = await Promise.all([
        listCustomers(sessionToken, true),
        getCustomerCapabilities(sessionToken),
      ]);
  ...
  async function saveCustomer(event: FormEvent) {
    ...
    if (!canManage || !formValid || busy) return;
    setBusy(true);
    setError(null);
    setFeedback(null);
    const common = {
  ...
  async function refreshFinancialDetail(customer: Customer) {
    const [creditSummary, entries, invoices, refundable] = await Promise.all([
      ...
      listRefundableCustomerPayments(sessionToken, customer.id),
    ]);
  ...
  async function openFinancialDetail(customer: Customer) {
    ...
    setError(null);
    try {
      await refreshFinancialDetail(customer);
    } catch (err) {
      setError(errorText(err));
    }
  }
  ...
  function openPaymentForm() {
    ...
    setPaymentNote('');
    setPaymentMethod(activeCashSession ? 'CASH' : 'BANK_TRANSFER');
    setPaymentOpen(true);
    setRefundOpen(false);
    setLastPayment(null);
  }
  ...
  async function submitPayment(event: FormEvent) {
    ...
    if (paymentMethod === 'CASH' && !activeCashSession) {
      setError(text.cashSessionRequired);
      return;
    }

    const allocations = openInvoices
      .map((invoice) => ({
        invoice_ledger_entry_id: invoice.invoice_ledger_entry_id,
        amount: allocationAmounts[invoice.invoice_ledger_entry_id] ?? '',
      }))
  ...
  function openRefundForm() {
    if (!canPostRefund || !selected || !openFiscalPeriod || refundablePayments.length === 0) return;
    const first = refundablePayments[0];
    setRefundPaymentId(first.payment_document_id);
    setRefundMethod(first.payment_method === 'CASH' && activeCashSession ? 'CASH' : 'BANK_TRANSFER');
    setRefundReason('');
    setRefundNote('');
    setManagerUsername('');
    setManagerPassword('');
  ...
  function chooseRefundPayment(value: string) {
    const id = value ? Number(value) : null;
    setRefundPaymentId(id);
    const payment = refundablePayments.find((row) => row.payment_document_id === id);
    if (payment) {
      setRefundMethod(payment.payment_method === 'CASH' && activeCashSession ? 'CASH' : 'BANK_TRANSFER');
    }
  }

  async function submitRefund(event: FormEvent) {
    event.preventDefault();
    if (!selected || !selectedRefundPayment || !openFiscalPeriod || busy) return;
    if (!managerUsername.trim() || !managerPassword || !refundReason.trim()) {
      setError(text.refundFieldsRequired);
      return;
    }
    if (refundMethod === 'CASH' && !activeCashSession) {
      setError(text.refundCashRequired);
      return;
    }
    ...
    try {
      const managerSession = await ipc.login(
      ...
      await authorizeCustomerPaymentRefund(managerToken, {
        authorization_id: authorizationId,
        source_payment_document_id: selectedRefundPayment.payment_document_id,
        refund_method: refundMethod,
        cash_session_id: refundMethod === 'CASH' ? activeCashSession?.id ?? null : null,
        reason: refundReason.trim(),
        ttl_minutes: 15,
      });

      const result = await postCustomerRefund(sessionToken, {
        request_id: crypto.randomUUID(),
        authorization_id: authorizationId,
        fiscal_period_id: openFiscalPeriod.id,
        document_date: currentBusinessDate(),
    ...
    <section className="sk-screen" data-testid="customers-screen">
      <header className="sk-screen__header">
        <div>
          ...
          {!loading && capabilities && !canManage ? (
            <small data-testid="customers-read-only">{text.readOnly}</small>
          ) : null}
        </div>
        {canManage ? (
          <button
      ...
      {feedback ? (
        <div className="sk-banner sk-banner--success sk-feedback-pop" role="status">
          {feedback}
        </div>
      ) : null}
        ...
        <form className="sk-card sk-form" onSubmit={saveCustomer} data-testid="customer-form">
          ...
          <div className="sk-form-grid">
            ...
            <label>
              ...
              <small className="sk-field-help">{text.taxIdHelp}</small>
            </label>
      ...
      {selected ? (
        <div className="sk-card" data-testid="customer-financial-detail">
            ...
            <div className="sk-action-group">
              {canPostPayment && openInvoices.length > 0 ? (
                <button
                  ...
                  data-testid="record-customer-payment"
                >
                  {text.payment}
                </button>
              ) : null}
              {canPostRefund && refundablePayments.length > 0 ? (
                <button
                  type="button"
                  className="sk-button sk-button--danger"
                  onClick={openRefundForm}
                  data-testid="refund-customer-payment"
                >
                  {text.refund}
                </button>
              ) : null}
          ...
          {lastPayment ? (
            <div className="sk-banner sk-banner--success" data-testid="customer-payment-success">
              {text.paymentPosted}: {lastPayment.document_number} · {lastPayment.amount}
            </div>
          ) : null}
          {lastRefund ? (
            <div className="sk-banner sk-banner--success" data-testid="customer-refund-success">
              {text.refundPosted}: {lastRefund.document_number} · {lastRefund.amount}
            </div>
          ) : null}
            ...
            <form className="sk-form sk-card" onSubmit={submitPayment} data-testid="customer-payment-form">
              ...
              <label>
                ...
                <select
                  ...
                  data-testid="customer-payment-method"
                >
                  <option value="CASH" disabled={!activeCashSession}>{text.cash}</option>
                  <option value="BANK_TRANSFER">{text.bank}</option>
                </select>
              </label>
              {!activeCashSession && paymentMethod === 'CASH' ? (
                <small>{text.cashSessionRequired}</small>
              ) : null}
              <h4>{text.openInvoices}</h4>
              {openInvoices.length === 0 ? <p>{text.noOpenInvoices}</p> : (
                <table className="sk-table">
                  ...
                  <tbody>
                    {openInvoices.map((invoice) => (
                      <tr key={invoice.invoice_ledger_entry_id}>
                        <td>{invoice.document_number ?? invoice.document_id ?? '—'}</td>
                        ...
                        <td>
                          <input
                            inputMode="decimal"
                            value={allocationAmounts[invoice.invoice_ledger_entry_id] ?? ''}
                            onChange={(event) => setAllocationAmounts((current) => ({
                              ...current,
                              [invoice.invoice_ledger_entry_id]: event.target.value,
                            }))}
                            data-testid={`payment-allocation-${invoice.invoice_ledger_entry_id}`}
                          />
                        </td>
                      </tr>
                    ))}
              ...
              <p>
                <strong>{text.paymentTotal}:</strong>{' '}
                <span data-testid="customer-payment-total">{paymentTotal}</span>
              </p>
              <label>
                {text.note}
              ...
              <div className="sk-form-actions">
                ...
                <button
                  type="submit"
                  className="sk-button sk-button--primary"
                  disabled={busy || Number(paymentTotal) <= 0}
                >
                  {text.postPayment}
                </button>
              </div>
          ...
          {refundOpen ? (
            <form className="sk-form sk-card" onSubmit={submitRefund} data-testid="customer-refund-form">
              <h3>{text.refund}</h3>
              <div className="sk-banner sk-banner--warning">{text.refundBoundary}</div>
              <label>
                {text.sourcePayment}
                <select
                  value={refundPaymentId ?? ''}
                  onChange={(event) => chooseRefundPayment(event.target.value)}
                  data-testid="customer-refund-source"
                >
                  {refundablePayments.map((payment) => (
                    <option key={payment.payment_document_id} value={payment.payment_document_id}>
                      {payment.document_number} · {payment.document_date} · {payment.amount}
                    </option>
                  ))}
                </select>
              </label>
              <label>
                ...
                <select
                  ...
                  data-testid="customer-refund-method"
                >
                  <option value="CASH" disabled={!activeCashSession}>{text.cash}</option>
                  <option value="BANK_TRANSFER">{text.bank}</option>
                </select>
              </label>
              {!activeCashSession && refundMethod === 'CASH' ? (
                <small>{text.refundCashRequired}</small>
              ) : null}
              <label>
                {text.refundReason} *
                <input
                  ...
                  data-testid="customer-refund-reason"
                />
              </label>
              <label>
                ...
                <input
                  ...
                  data-testid="customer-refund-manager-username"
                />
              </label>
              <label>
                ...
                <input
                  ...
                  data-testid="customer-refund-manager-password"
                />
              </label>
              <label>
                {text.note}
                <input value={refundNote} onChange={(event) => setRefundNote(event.target.value)} />
              </label>
              <div className="sk-form-actions">
                <button
                  type="button"
                ...
                <button
                  type="submit"
                  className="sk-button sk-button--danger"
                  disabled={busy || !selectedRefundPayment}
                  data-testid="post-customer-refund"
                >
                  {text.postRefund}
                </button>
              </div>
          ...
          {openInvoices.length === 0 ? <p>{text.noOpenInvoices}</p> : (
            <table className="sk-table" data-testid="open-customer-invoices">
              <thead>
                <tr>
                  <th>{text.document}</th>
                  <th>{text.due}</th>
          ...
          <h3>{text.refundablePayments}</h3>
          {refundablePayments.length === 0 ? <p>{text.noRefundablePayments}</p> : (
            <table className="sk-table" data-testid="refundable-customer-payments">
              <thead>
                <tr>
                  <th>{text.document}</th>
                  <th>{text.created}</th>
              ...
              <tbody>
                {refundablePayments.map((payment) => (
                  <tr key={payment.payment_document_id}>
                    <td>{payment.document_number}</td>
                    <td>{payment.document_date}</td>
                    <td>{payment.payment_method === 'CASH' ? text.cash : text.bank}</td>
          ...
          <table className="sk-table" data-testid="customer-ledger-table">
            ...
            <tbody>
              {ledger.length === 0 ? (
                <tr><td colSpan={5}>{text.noLedger}</td></tr>
              ) : ledger.map((entry) => (
                <tr key={entry.id}>
