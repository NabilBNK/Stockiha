...
'@
    ...
    Run-Step 'S4-001 credit-sale regression' {
        psql $adminUrl -X -v ON_ERROR_STOP=1 -f src-tauri/tests/receivables/s4_001_credit_sale_integration.sql
    }
    Run-Step 'S4-001 customer-payment regression' {
        psql $adminUrl -X -v ON_ERROR_STOP=1 -f src-tauri/tests/receivables/s4_001_customer_payment_integration.sql
    }
    Run-Step 'S4-002 cashier lifecycle DB integration' {
        psql $adminUrl -X -v ON_ERROR_STOP=1 -f src-tauri/tests/cash/s4_002_cash_session_lifecycle.sql
    }
    Run-Step 'S4-002 cash ownership DB integration' {
        psql $adminUrl -X -v ON_ERROR_STOP=1 -f src-tauri/tests/cash/s4_002_cash_session_ownership_integration.sql
    }

    $bash = Get-Command bash -ErrorAction SilentlyContinue
    if ($null -eq $bash) {
        throw 'bash is required for the cash-session and credit concurrency harnesses. Install Git for Windows/WSL or run the scripts from a bash-capable environment.'
    }

    Run-Step 'S4-002 cash-session close concurrency race' {
        bash src-tauri/tests/cash/s4_002_cash_session_concurrency.sh
    }
    Run-Step 'S4-001 credit-limit concurrency regression' {
        bash src-tauri/tests/receivables/s4_001_credit_concurrency.sh
    }
}

Write-Host "`nS4-002 automated verification completed successfully." -ForegroundColor Green
Write-Host 'Manual Tauri/Windows blind-count, approval, handover, multilingual and touchscreen smoke testing is still required before merge.' -ForegroundColor Yellow
