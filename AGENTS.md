...
# Prototype MVP Continuation Rule

The current Stockiha application is a functional prototype MVP, not the final
production product.

Completion of the Golden Transaction Chain proves the core architecture and
workflow, but does not terminate the planned development slices.

Agents must continue implementing the remaining tasks in `TASKS.md` and
`final-architecture.md`.

## UI status

The current interface is provisional.

Do not pause planned business-feature development for broad visual redesign.

UI work is required immediately only when an issue:

- blocks a required workflow
- causes incorrect or misleading business information
- creates a security or privacy problem
- makes touchscreen operation impractical
- causes crashes, data loss, or failed transactions

Cosmetic refinement, advanced responsiveness, animations, visual branding,
dashboard polish, and final information density are deferred to a dedicated
UI/UX stabilization slice.

When touching an existing screen for a new feature, agents may make small,
localized usability improvements, but must not expand the task into a complete
redesign.

## Production boundaries

Although the UI is provisional, backend integrity is not provisional.

Agents must continue enforcing:

- exact decimal arithmetic
- warehouse-specific WAC
...
Prototype status must never be used to justify fake persistence, mock production
flows, weakened constraints, hardcoded credentials, or temporary parallel
architectures.
...
1. Read `TASKS.md`, `CURRENT_SLICE.md`, and the relevant architecture sections.
2. Reconcile tasks already implemented by previous batches.
3. Select the first genuinely incomplete task.
4. Create one implementation branch for the next coherent feature batch.
5. Continue development without waiting for final UI polish.